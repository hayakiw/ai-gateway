...
> 利用シーンに応じて対象は追加・調整する前提です。詳細は [運用・保守メモ](./operations.md) の「拡張ポイント」を参照。
...
spaCy NER だけでは取りこぼしが出るため、以下の `PatternRecognizer` を `ja` / `en` 両方に追加登録しています。

- **EMAIL_ADDRESS**: 厳格メール正規表現
- **CREDIT_CARD**: 13〜19 桁連続数字（日本語文字境界対応のため `\b` ではなく前後の数字 lookaround を使用）
- **LOCATION**: 日本語住所（都道府県 + `郡`? + 市区町村 + 番地）
- **PERSON**: 敬称（さん/サン/様/さま/氏/君/くん/ちゃん/先生/殿）を直後に伴う漢字・カタカナ列

また、マイナンバー・運転免許証・パスポート・銀行口座については文脈手がかり（キーワード + 区切り + 数字）で抽出する正規表現を `CONTEXTUAL_REGEXES` に定義しています。

### マスキング時のルール

- Presidio の検知結果はスコア降順で並べ替える
- 重なり合う候補は高スコア側を優先し、低スコア側を捨てる
- PERSON / LOCATION は英字・ひらがな・カタカナ・漢字を含まない記号のみの誤検出（例: `#`）を除外
- 最終的な採用順はテキスト内の出現位置順に並べ直す
- 同じ元文字列が複数回出現した場合は同じプレースホルダーを再利用する
- PERSON に限り「片方が他方を包含する表記」は同一人物とみなし同じプレースホルダーを共有（例: `田中太郎` と `田中` は `[PERSON_1]`）。長い表記をマッピングの正式値として採用する
